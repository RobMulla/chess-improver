...
class TestGameAnalysisIntegration:
    ...
    def setup_method(self, method):
        ...
        session = get_session()
        try:
            # Ensure tables exist (in case using memory or fresh file)
            ...
            session.execute(
                text(
                    "DELETE FROM positions WHERE game_id IN (SELECT id FROM games WHERE game_id LIKE 'TEST_%')"
                )
            )
...
1. e4 e5 2. Bc4 Nc6 3. Qh5 Nf6 4. Qxf7# 1-0"""
        ...
        with GameAnalyzer() as analyzer:
            ...
            # White scored checkmate but used Scholar's Mate (dubious), so accuracy won't be 100%
            # Engines punish early Queen moves heavily
            assert result["player_accuracy"] >= 40.0
...
1. e4 e5 2. Nf3 Nc6 3. Bc4 Bc5 4. d3 Nf6 5. O-O O-O *"""
        ...
        with GameAnalyzer() as analyzer:
            ...
            # With Win% algorithm, even good moves might not reach 100% due to engine noise
            assert result["player_accuracy"] >= 85.0  # At least 85%
            assert result["opening_accuracy"] >= 85.0
            ...
            # assert result['player_accuracy'] == 100.0  # Relaxed check for float precision
...
class TestAccuracyFormula:
    ...
    def test_one_big_blunder(self):
        ...
        moves = [{"classification": "best", "eval_drop": 0}] * 9 + [
            {"classification": "blunder", "eval_drop": 300}
        ] * 1
        accuracy = MoveClassifier.calculate_accuracy_from_moves(moves)
