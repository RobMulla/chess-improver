...
class MoveClassifier:
    ...
    def classify_move(
        ...
        """
        from src.analysis.win_chance import classify_by_win_loss, win_chance_loss
        ...
        # NOTE: "Great" move detection disabled
        # Chess.com uses complex heuristics we can't easily replicate
        # (e.g., only good move in difficult position, defensive resources, etc.)
        # For now, stick to Win% based classification only
        #
        # if result["classification"] == "best":
        #     position_complexity = abs(best_eval - prev_eval)
        #     if position_complexity > 800:
